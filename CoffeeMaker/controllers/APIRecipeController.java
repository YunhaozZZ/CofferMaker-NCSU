...
public class APIRecipeController extends APIController {
    ...
    /**
     * REST API method to allow editing a recipe in the Coffee Maker System
     *
     * @param recipe
     *            - the recipe to replace with
     * @return HTTP 200 if all was well, else an error
     */
    @PutMapping ( BASE_PATH + "/recipes" )
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe ) {

        final Recipe db = service.findByName( recipe.getName() );
        if ( db == null ) {
            return new ResponseEntity( errorResponse( "No recipe found: " + recipe.getName() ), HttpStatus.NOT_FOUND );
        }

        db.updateRecipe( recipe );
        service.save( db );
        return new ResponseEntity( successResponse( recipe.getName() + " updated successfully" ), HttpStatus.OK );
    }
}
